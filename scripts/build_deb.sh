...
mkdir -p "$OUTPUT_DIR"
rm -f "$OUTPUT_PACKAGE" 2&>/dev/null
...
if [ "$(which dpkg-sig 2>/dev/null || true)" == "" ]; then
  echo "Warning: dpkg-sig not found, skipping deb signing." >&2
  exit 0
fi

# sign the deb
dpkg-sig --sign builder -k "$DEBKEY" "$OUTPUT_PACKAGE"

# TODO: verify the signed deb? (needs ppa installed, etc)
# dpkg-sig --verify "$OUTPUT_PACKAGE"
