...
#define DMA_ARC_HS_NP_ATTR       BIT(3) /* Enable non posted writes */
#define DMA_ARC_HS_SET_DONE_ATTR BIT(0) /* Set done without triggering interrupt */
...
static inline void dma_arc_hs_start_hw(uint32_t dma_ch, const void *p_src, void *p_dst, uint32_t len,
				    uint32_t attr)
{
...
#define DMA_ARC_HS_MAX_CHANNELS    16
#define DMA_ARC_HS_MAX_DESCRIPTORS 256
#define DMA_ARC_HS_ATOMIC_WORDS    ATOMIC_BITMAP_SIZE(DMA_ARC_HS_MAX_CHANNELS)

struct dma_arc_hs_channel {
	uint32_t id;
	bool in_use;
	bool active;
...
struct dma_arc_hs_data {
	struct dma_context dma_ctx;
	struct dma_arc_hs_channel channels[DMA_ARC_HS_MAX_CHANNELS];
	atomic_t channels_atomic[DMA_ARC_HS_ATOMIC_WORDS];
...
{
	const struct dma_arc_hs_config *dev_config = dev->config;
	struct dma_arc_hs_data *data = dev->data;
	struct dma_arc_hs_channel *chan;
...
static int dma_arc_hs_start(const struct device *dev, uint32_t channel)
{
	...
	attr = DMA_ARC_HS_SET_DONE_ATTR | DMA_ARC_HS_NP_ATTR;
	...
	dma_arc_hs_start_hw(channel, (const void *)block->source_address, (void *)block->dest_address,
...
static void dma_arc_hs_completion_thread(void *arg1, void *arg2, void *arg3)
{
	...
	const struct dma_arc_hs_config *config = dev->config;
	struct dma_arc_hs_data *data = dev->data;
	int i;
...
static int dma_arc_hs_init(const struct device *dev)
{
	...
	k_thread_name_set(data->completion_thread_id, "dma_arc_hs_completion");
...
#define DMA_ARC_HS_INIT(inst)                                                                         \
	static const struct dma_arc_hs_config dma_arc_hs_config_##inst = {                               \
		.base = DMA_AUX_BASE, /* not in in addressable memory */ \
		...
		.coherency_support = DT_INST_PROP(inst, coherency_support),              \
                                                                                                   ...
                                                                                                   \
	static struct dma_arc_hs_data dma_arc_hs_data_##inst;                                            \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, dma_arc_hs_init, dma_arc_hs_cleanup_wrapper, &dma_arc_hs_data_##inst,   \
			      &dma_arc_hs_config_##inst, POST_KERNEL, CONFIG_DMA_INIT_PRIORITY,       \
			      &dma_arc_hs_api);

DT_INST_FOREACH_STATUS_OKAY(DMA_ARC_HS_INIT)
